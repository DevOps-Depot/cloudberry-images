...
source "amazon-ebs" "base-cbdb-test-image" {
  # AWS credentials
...
build {
  sources = ["source.amazon-ebs.base-cbdb-test-image"]

  # Provisioner to add CBDB test RPM dependencies
  provisioner "shell" {
    script = "scripts/system_add_cbdb_test_rpm_dependencies.sh"
  }
